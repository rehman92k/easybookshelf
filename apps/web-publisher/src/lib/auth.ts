...
export async function refreshAccessTokenFromBearer(): Promise<boolean> {
...
export async function fetchCurrentUser(): Promise<AuthLoginResponse['user'] | null> {
  ...
  async function loadMe(): Promise<Response> {
  ...
  if (response.status === 401) {
    const refreshed =
      (await refreshAccessTokenFromBearer()) || (await refreshAccessToken());
    if (!refreshed) {
