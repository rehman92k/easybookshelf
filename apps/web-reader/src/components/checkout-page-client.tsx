...
import type { CheckoutSession, OrderItemType } from '@easybookshelf/shared-types';
...
const VALID_TYPES: OrderItemType[] = ['purchase', 'rental_15', 'rental_30'];

function parseType(value: string | null): OrderItemType {
  if (value && VALID_TYPES.includes(value as OrderItemType)) {
    return value as OrderItemType;
  }
  return 'purchase';
}
...
export function CheckoutPageClient({ slug }: CheckoutPageProps) {
  ...
  const orderType = parseType(searchParams.get('type'));
  ...
  const loadCheckout = useCallback(async () => {
    ...
    try {
      const order = existingOrderId
        ...
        : await createOrder({ bookSlug: slug, type: orderType });
  ...
  }, [slug, orderType, existingOrderId]);

  useEffect(() => {
    if (authLoading) return;
    if (!user) {
      router.replace(`/login?next=${encodeURIComponent(`/books/${slug}/checkout?type=${orderType}`)}`);
      return;
    }
    void loadCheckout();
  }, [authLoading, user, router, slug, orderType, loadCheckout]);
  ...
  async function handleRazorpayPay() {
    ...
    try {
      ...
      const options = {
        ...
        name: 'EasyBookshelf',
        description: `${orderTypeLabel(item.type)} — ${item.bookTitle}`,
      ...
      <main className="mx-auto max-w-lg px-6 py-10">
        ...
        {session && item && !done ? (
          <div className="mt-8 rounded-xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-900">
            ...
            <p className="mt-1 text-stone-600 dark:text-stone-400">{orderTypeLabel(item.type)}</p>
